 ...
 *
 * This class implements the ValidCardHand interface so it must implement the methods in it
 *
 * This class is abstratic because we don't want anyone to instantiate a generic CardHand
 * so it can only be a super class
 *
 * The abstractic class is defining basic data and behaviors that subclasses can override
 *
 ***************************************************************************************************/
public abstract class CardHand   implements ValidCardHand  { // since this implements the ValidCardHand interface 
	// it has to implement the methods of the ValidCardHand interface
...
@Override     // Ask compiler to check to be sure this is a valid override of superclass or interface method	
	public void show() {                // Display each card in Hand
		for (PlayingCard aCard : aHand)
		{
			...
			return; // optional return since the method is void, makes the code look nicer
	}
